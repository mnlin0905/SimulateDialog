 ...
 */
final class PageManager extends TransitionParent implements ShadeMaskView.MaskOperateListener, PageMethodPiling {
    ...
    final boolean removePage(@NotNull Page page) {
        ...
        if (page.getPageAppearanceType() == PageAppearance.PAGE_APPEARANCE_PART && page instanceof PartPage) {
            Page mask = ((PartPage) page).peekMaskForPart();
            if (mask != null) {
                removePage(mask);
    ...
    final void insertPage(int index, @NotNull Page page) {
        ...
        if (isRealOperatePage) {
            ...
            Log.v(TAG, "*****************");// on-page-resume (The interface inserted in the middle will not trigger this method)
    ...
    private void insertMaskPage(PartPage host, int index) {
        Page mask = host.peekMaskForPart();
        if (mask == null) {
