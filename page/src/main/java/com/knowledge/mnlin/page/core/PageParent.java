...
public class PageParent extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        // enter - point
        addPage(injectPageEnterPoint());
    }

    /**
     * inject the enter-point of application
     * <p>
     * if not implement ,it will invoke reflect
     */
    protected Page injectPageEnterPoint() {
        try {
            Class<?> clazz = Class.forName(PageGenPackageConst.PAGE_MODULE_MAIN_CORE_PACKAGE + "." + PageGenClassConst.CLASS_PAGE_GEN_ENTER_POINT);
            Method declaredMethod = clazz.getDeclaredMethod(PageGenMethodConst.PAGE_ENTER_POINT_METHOD);
            return ((Page) declaredMethod.invoke(clazz));
        } catch (Exception e) {
            throw new RuntimeException("setup enter-point error: " + e.getMessage());
        }
    }
